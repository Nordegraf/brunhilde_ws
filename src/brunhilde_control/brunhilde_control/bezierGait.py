...
class BezierGait:
    ...
    def __init__(self, dSref=[0.5, 0.0, 0.0, 0.5], dt=0.01, Tswing=0.2):
        self.dSref = dSref
        self.dt = dt
        self.Tswing = Tswing
        self.reset()
    ...
    def bezier_swing(self, phase, L, LateralFraction, clearance_height=0.04):
    ...
    def sine_stance(self, phase, L, LateralFraction, penetration_depth=0.05):
        X_POLAR = np.cos(LateralFraction)
        Y_POLAR = np.sin(LateralFraction)
    ...
    def swing_step(self, phase, L, T_bf, index):
        phaseYAW = phase * 1.2 if self.StanceSwing == self.SWING else phase
        stepX, stepY, stepZ = self.bezier_swing(phase, L, self.yaw_circle(T_bf, index))
        stepX -= T_bf.x
        stepY -= T_bf.y
        stepZ -= T_bf.z
        return stepX, stepY, stepZ

    def stance_step(self, phase, L, T_bf, index):
        stepX, stepY, stepZ = self.sine_stance(phase, L, self.yaw_circle(T_bf, index))
        stepX -= T_bf.x
        stepY -= T_bf.y
        stepZ -= T_bf.z
        return stepX, stepY, stepZ

    def get_foot_step(self, phase, stance_swing, L, T_bf, index):
        if stance_swing == self.SWING:
            return self.swing_step(phase, L, T_bf, index)
        else:
            return self.stance_step(phase, L, T_bf, index)

    def generate_trajectory(self, T_bf, L, vel, dt):
        ...
        for i in range(4):
            ...
            stepX, stepY, stepZ = self.get_foot_step(phase, stance_swing, L, T_bf[i], i)
            foot_step_vector = Vector3()
            foot_step_vector.x = stepX
            foot_step_vector.y = stepY
            foot_step_vector.z = stepZ
            foot_positions.append(foot_step_vector)
